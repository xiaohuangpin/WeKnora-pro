...
def get_backend() -> str:
        """
        根据设备类型和 GPU 显存大小返回对应的 backend：
        - CPU → 'pipeline'
        - GPU 且显存 < 12GB → 'pipeline'
        - GPU 且 8GB ≤ 显存 < 16GB → 'vlm-transformers'
        - GPU 且显存 ≥ 16GB → 'vlm-vllm-engine'
        """
        if not torch.cuda.is_available():
            return "pipeline"  # 使用 CPU
...
async def parse_pdf(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            logger.info("Cleaning up memory after inference")
            clean_memory(device)
...
@click.option('--port', default=7776, type=int, help='Server port (default: 7776)')
...
#python web_api.py --host 127.0.0.1 --port 7456 --reload True